...
繁問繁答與繁問簡答使用 `--reeval` 修正評估邏輯（中文數字正規化 + 同義詞比對）。
簡問簡答（†）目前僅有原始 `is_correct` 評估，實際準確率可能高於表中數值（詳見下方說明）。

| 模型 | 架構 | 繁問繁答 | @130K | 繁問簡答 | 簡問簡答† | @130K† | Tokenizer Overhead |
|------|------|---------|-------|---------|---------|--------|-------------------|
| gemma4:31b | Dense 31B | **100.0%** | **100.0%** | — | 99.9% | 99.1% | +5.7% |
| qwen3.5:35b | Dense 35B | **100.0%** | **100.0%** | — | 100.0% | 100.0% | +10.3% |
| qwen3.5:27b | Dense 27B | **100.0%** | **100.0%** | — | 99.8% | 100.0% | +10.3% |
| qwen3.5:9b | Dense 9B | **100.0%** | **100.0%** | — | 99.8% | 98.2% | +10.3% |
...
| qwen3.5:2b | Dense 2B | 99.8% | 99.1% | — | 96.7% | 100.0% | +10.3% |
| gemma4:26b | Dense 26B | 98.9% | 91.8% | — | 99.5% | 95.5% | +5.7% |
| gemma3:12b | Dense 12B | 98.0% | 86.4% | — | 93.7%† | 60.0%† | +5.7% |
| llama3.3:70b | Dense 70B | 97.9% | 80.9% | — | 98.6%†‡ | —‡ | +7.1% |
| llama3.1:8b | Dense 8B | 97.1% | 91.8% | **98.8%** | 91.5%† | 68.2%† | +7.1% |
| qwen3:8b | Dense 8B | 94.1%（≤65K）| —（超限）| — | — | — | — |
| gemma3:4b | Dense 4B | 91.6% | 52.7% | **99.1%** | 87.7%† | 38.2%† | +5.7% |
| gemma3:27b | Dense 27B | 85.0%（★） | 0.0%（★）| — | 77.0%† | 0.0%† | +5.7% |
| gemma3:1b | Dense 1B | 76.4%（≤32K）| — | — | 78.5%†（≤32K）| — | +5.7% |

★ gemma3:27b 在 100K+ 輸出退化為 `<pad>` token，非正常 context rot，數值不代表理解能力。
† 簡問簡答欄位使用**原始** `evaluation.is_correct`，尚未套用 `--reeval` 修正，實際準確率可能被低估。
‡ llama3.3:70b 簡問簡答僅完成 392 筆（≤6K 字元），不具代表性。

> 繁/簡直接比較需謹慎：trad 已套用 reeval，simp_q 尚未，評估不對稱。詳見下方量化分析。
...
繁問繁答（trad）與繁問簡答（simp）的準確率透過 `04_analyze.py --reeval` 重新評估，套用中文數字正規化與同義詞比對。簡問簡答（simp_q，h2_ 檔案）因 `04_analyze.py` 目前不支援 `simplified_q` variant，**所有 simp_q 數值均為原始 `is_correct`，尚未套用 reeval**。

實際影響：以 gemma3:4b 為例，trad 在 65K 的原始準確率約 73.6%，reeval 後為 88.2%（差距約 14.6pp）。若 simp_q 同樣套用 reeval，其準確率亦會提高，繁/簡的直接差距將改變。**在 h2_ reeval 修復前，所有繁/簡比較結論均需保留此不確定性**。
...
| 項目 | 影響 | 優先級 |
|------|------|-------|
| 修復 `04_analyze.py` 對 `simplified_q` variant 的 --reeval 支援 | 所有繁/簡比較結論的可信度 | **高** |
| llama3.3:70b simp_q 補完（目前僅 392 筆，≤6K 字元）| 70B 的繁/簡比較 | 高 |
