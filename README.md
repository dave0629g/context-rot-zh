...
### 已知限制：Haystack 中的幹擾資訊

部分 Wikipedia 語料中包含與 needle 相似的真實資訊，可能導致模型找到真實段落而非我們插入的 needle：
...
- 幹擾對三種 variant 的影響是**對等的**（同一個 haystack，同一個幹擾源），因此 variant 之間的相對比較仍然有效
- N04 的絕對準確率偏低是幹擾造成的，不代表模型能力較差
- 錯誤分析中 N04 出現的「4350萬」「150隻」「60-80隻」等幻覺數字，可能源自模型找到了 haystack 中真實的黑熊相關段落

**未來改進：** 建構 haystack 前應增加幹擾檢測步驟，確認語料中不含與 needle 相似的資訊。
