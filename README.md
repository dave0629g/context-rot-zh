...
| llama3.3:70b simp_q 補完（目前僅 502 筆，≤6K 字元）| 70B 的繁/簡比較 | **高** |
| gemma3:1b 補足 65K+ 數據（目前僅至 32K）| 1B 模型的完整 context rot 曲線 | 中 |
| qwen3:8b simp_q 執行（目前無數據）| 8B Qwen3 的繁/簡比較 | 中 |
...
16 個模型繁問繁答 + 15 個模型簡問簡答均已完成並套用 reeval（評估標準一致）。2 個模型（gemma3:4b, llama3.1:8b）另有繁問簡答資料。本章節就兩個核心研究問題和探索性分析提出正式答覆。

---

### RQ1：Token 膨脹是否使語境腐化的時機點（onset）提前？

**定義**：onset = 準確率開始穩定下滑的 context 長度（連續兩點 < 95% 或單點 < 80%）。若 token 計數是驅動因子，則繁體 onset（字元數）× token overhead ratio 應 ≈ 簡體 onset 的 token 位置。

**分析基礎**：gemma3:4b 與 llama3.1:8b 為主要證據來源（完整三 variant），15 個模型的繁問繁答 vs 簡問簡答提供方向性佐證。

#### 65K 字元下三 Variant 對比（reeval，評估標準一致）

| 模型 | 繁問繁答 | 繁問簡答 | 簡問簡答 | 簡答−繁答 | @65K trad tokens | @65K simp_q tokens | overhead |
|------|---------|---------|---------|----------|-----------------|-------------------|---------|
| gemma3:4b | 88.2% | 93.6% | 92.7% | **+4.5pp** | 49,062 | 46,348 | +5.9% |
| llama3.1:8b | 92.7% | 94.5% | 96.4% | **+3.6pp** | 57,248 | 53,399 | +7.2% |

**三 variant 排序符合假說預期**：兩個模型在 65K 均呈現 簡問簡答 ≈ 繁問簡答 > 繁問繁答，簡體 context 比繁體 context 提升 1.8–5.4pp。繁問簡答與簡問簡答的差異很小（0.9–1.9pp），表示問題語言的影響遠小於 context 語言。

**onset 比較**：gemma3:4b 繁問繁答在 12K 字元開始滑落（99.1%），簡問簡答在 16K 仍維持 98.2%；llama3.1:8b 兩者在 65K 才開始明顯滑落，onset 差距不顯著。繁體 onset × 1.059 並未精確對齊簡體 onset token 位置——token 計數是加重因子，但非唯一驅動因子。

**RQ1 答覆**：繁體輸入的額外 token 在中等 context 長度（65K 字元）使退化提前或加重 1.8–5.4pp，方向一致支持假說。但 onset 位置未精確對齊 token overhead ratio，表示 token 膨脹僅為部分解釋因子，架構設計、訓練分布等因素亦有顯著作用。

---

### RQ2：Token 膨脹是否影響腐化速率（rate）？

**定義**：rate = 退化區間內準確率對 token 數的線性迴歸斜率（pp/1k tokens）。onset（時機點）與 rate（速率）為獨立可量測變數。

#### 完整退化曲線比較

**gemma3:4b**（退化最清晰的 benchmark 模型）：

| 字元數 | 繁問繁答 | 簡問簡答 | 差距 | 觀察 |
|--------|---------|---------|------|------|
| 12K | 99.1% | 100.0% | −0.9pp | 繁體略早開始滑落 |
| 65K | 88.2% | 92.7% | −4.5pp | 差距最大 |
| 100K | 68.2% | 68.2% | **0pp** | 差距歸零 |
| 130K | 52.7% | 52.7% | **0pp** | 差距歸零 |

→ 結果屬 **(a) 僅時機點提前（曲線左移）**，非速率加快。到重度 rot 區間（100K+），繁/簡 token 差異（2,714 tokens）相對於總 token 數（~75K+）已不構成顯著差異。

**llama3.1:8b**（非單調行為）：

| 字元數 | 繁問繁答 | 簡問簡答 | 差距 | 觀察 |
|--------|---------|---------|------|------|
| 65K | 92.7% | 96.4% | −3.6pp | 符合假說 |
| 100K | 90.9% | 91.8% | −0.9pp | 差距縮小 |
| 130K | 91.8% | 82.7% | **+9.1pp** | **反轉** |

→ **無法歸類為 (a)(b)(c) 任一種**。130K 的反轉（繁體反勝 9.1pp）是重要反例，可能涉及訓練分布偏差（LLaMA3 繁體訓練資料比例較高？）或 130K 字元區間的統計噪音（每長度僅 110 筆），列為待調查項目。

**位置敏感度——質性佐證**：在 gemma3:4b，繁問繁答呈現明顯的「Lost in the Middle」效應（位置 0.5 準確率 0.81）；繁問簡答（簡體 haystack）的位置曲線幾乎平坦（0.98–1.0）。相同問題語言、不同 haystack 腳本，產生截然不同的注意力分布，支持 tokenizer overhead 加劇了中段注意力稀釋。

**RQ2 答覆**：gemma3:4b 的退化模式為曲線左移（onset 提前），非速率加快——繁/簡的斜率在重度 rot 區間趨同。llama3.1:8b 的 130K 反轉現象目前無法解釋，列為待調查項目。

---

### 探索性分析：架構、參數量與繁體長 Context 抵抗力

以下為探索性分析（非確認性結論），樣本數不足做統計迴歸，但可觀察趨勢。16 個模型依退化曲線形態歸納為四種行為模式。
...
### 探索性分析：Tokenizer Overhead 與性能退化的量化關係
...
1. **架構世代是壓倒性的主因，tokenizer overhead 最多是次要加重因子**：Qwen3.5 以最高 overhead（+10.3%）完全免疫，Gemma4 亦然。架構升級帶來的改善（gemma3:4b → gemma4:e2b 提升 47pp @130K）遠大於繁/簡差異（1.8–5.4pp）。

2. **在已脆弱的模型中，繁體確實加速退化（65K 字元）**：gemma3:4b（+4.5–5.4pp）和 llama3.1:8b（+1.8–3.6pp）方向一致，三 variant 排序符合假說預期（簡問簡答 ≈ 繁問簡答 > 繁問繁答）。

3. **效應在極長 context 下非單調**：gemma3:4b 在 100K/130K 繁/簡差距歸零；llama3.1:8b 在 130K 出現反轉（trad 91.8% > simp_q 82.7%，差 +9.1pp）。Token overhead 效應不是在所有 context 長度下線性累積。

4. **位置敏感度差異是質性確認**：gemma3:4b 的繁問繁答在中段位置呈現顯著的「Lost in the Middle」效應，而繁問簡答（簡體 haystack）的位置曲線幾乎平坦。

5. **模型行為模式分類（四類）**：硬性上限型（模式 A）、漸進退化型（模式 B）、突發崩潰型（模式 C）、近乎完美型（模式 D），有助於研究者選擇適合的模型作為研究工具或生產部署。

#### 未建立的項目

- **Overhead-Performance 定量關係**：僅 2 個有效數據點，且 Qwen3.5 反例直接反駁「overhead 越高 rot 越嚴重」的簡單因果。
- **llama3.1:8b @130K 反轉的機制**：簡體 context 反而更差，可能涉及訓練分布偏差或特定 tokenizer 的長 context 行為差異，但無法從現有數據區分。
- **效應的普遍性**：context rot 假說僅在 2 個模型的 65K 區間直接驗證。
- **Qwen3.5 / Gemma4 的真實 context rot 臨界點**：天花板效應使本實驗設計對這些模型失效。
...
1. **補完 llama3.3:70b simp_q**（目前僅 502 筆，≤6K 字元）——70B 的繁/簡比較需要完整數據。
2. **調查 llama3.1:8b @130K 反轉現象**——trad 91.8% > simp_q 82.7%（差距 +9.1pp），方向違反假說。需確認是否為統計噪音（僅 110 筆 @130K），或是否在更多模型中重現。

**中期：**

3. **擴充三 variant 至 Gemma4 / Qwen3.5**，使用 200K+ 字元，克服天花板效應，取得更多 RQ1/RQ3 的數據點。
4. **英文 baseline 實驗**：將 haystack 翻譯成英文，建立模型長 context 能力的理論上限參考。英文 token 效率比繁體高約 44–48%，但因訓練資料分布、語義密度等混淆因子過多，僅作為附錄參考，不用於驗證 tokenizer overhead 假說。
5. **句段級 tokenizer overhead 量化**：對實驗用的 haystack 語料直接測量，取代語料級平均值。
6. **修復實驗設計缺陷**：加入 distractor detection（N04 類問題）、sanity probe（模型輸出退化偵測）、token count check（context window 超限預警），寫入 v2 實驗腳本。
7. **控制詞彙實驗**：使用完全對應的繁簡版本（字詞一對一轉換，不調整語序），消除語義混淆，更純粹地測試 tokenizer 效應。
...
| Ch.3 研究方法 | 三 variant 設計、reeval 邏輯、token overhead 實測（字元級 14.5% vs prompt 級 5.9–10.6%） | ✅ 已有 |
| Ch.4.1 RQ1 | Token 膨脹是否使腐化時機點（onset）提前？ | ✅ 完整（gemma3:4b + llama3.1:8b 三 variant） |
| Ch.4.2 RQ2 | Token 膨脹是否影響腐化速率（rate）？ | ✅ 完整（含 130K 反轉待解釋） |
| Ch.4.3 探索性分析 | 架構世代、Dense vs MoE、反 scaling、宣稱 vs 實際 ctx | ✅ 16 模型 trad + 15 模型 simp_q |
| Ch.5 討論 | token 碎片化雙效應（token 數 + 語意密度）不可分離、latent pivot、架構掩蓋效應 | ✅ 定性論述 |
| Ch.6 結論 | 假說在脆弱模型中成立但效果溫和、架構世代是壓倒性主因 | ✅ |
| 附錄 | 16 模型完整退化曲線、熱力圖、模型適用性矩陣 | ✅ 已有 |
